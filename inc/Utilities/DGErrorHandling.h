...
#define DG_ERROR_TYPE_LIST                                                                                           \
	...
	_( VALIDATION_ERROR,                                                                                             \
	   "[VALIDATION]",                                                                                               \
	   "validation error: exception is thrown, but error is NOT registered in global error collection" )             \
	_( RUNTIME_ERROR,                                                                                                \
	   "[ERROR]",                                                                                                    \
	   "regular runtime error: exception is thrown, and error is registered in global error collection" )            \
...
#define DG_WARNING( err_msg, err_code )                                                                        \
	do                                                                                                         \
	{                                                                                                          \
		DG::ErrorHandling::warningAdd( __FILE__, TOSTRING( __LINE__ ), FUNCTION_NAME, DG::err_code, err_msg ); \
	} while( 0 )

/// Report and throw runtime error
#define DG_ERROR( err_msg, err_code )     \
	do                                    \
	{                                     \
...
#define DG_VALIDATION_ERROR( err_msg )       \
	do                                       \
	{                                        \
		DG::ErrorHandling::errorAdd(         \
			__FILE__,                        \
			TOSTRING( __LINE__ ),            \
			FUNCTION_NAME,                   \
			DG::ErrorType::VALIDATION_ERROR, \
			DG::ErrCompilerBadState,         \
			err_msg );                       \
	} while( 0 )

/// Report and throw critical error, set critical flag
#define DG_CRITICAL_ERROR( err_msg, err_code ) \
	do                                         \
	{                                          \
		DG::ErrorHandling::errorAdd(           \
			__FILE__,                          \
...
/// Assertion macro
	...
	#define DG_ASSERT( expr ) \
		DG::ErrorHandling::assertHandle( __FILE__, TOSTRING( __LINE__ ), FUNCTION_NAME, #expr, expr )
#endif

#define DG_ERROR_TRUE( expr )         \
	DG_CHECK_HELPER_BOOL_(            \
		( expr ),                     \
		#expr,                        \
		false,                        \
		DG::ErrorType::RUNTIME_ERROR, \
...
#define DG_ERROR_FALSE( expr )        \
	DG_CHECK_HELPER_BOOL_(            \
		...
		ErrAssert,                    \
		DG::ErrorHandling::errorAdd )
#define DG_ERROR_EQ( expr1, expr2 ) \
	DG_CHECK_HELPER_2_( expr1, expr2, EQ, DG::ErrorType::RUNTIME_ERROR, ErrAssert, DG::ErrorHandling::errorAdd )
#define DG_ERROR_NE( expr1, expr2 ) \
	DG_CHECK_HELPER_2_( expr1, expr2, NE, DG::ErrorType::RUNTIME_ERROR, ErrAssert, DG::ErrorHandling::errorAdd )
#define DG_ERROR_LT( expr1, expr2 ) \
	DG_CHECK_HELPER_2_( expr1, expr2, LT, DG::ErrorType::RUNTIME_ERROR, ErrAssert, DG::ErrorHandling::errorAdd )
#define DG_ERROR_GT( expr1, expr2 ) \
	DG_CHECK_HELPER_2_( expr1, expr2, GT, DG::ErrorType::RUNTIME_ERROR, ErrAssert, DG::ErrorHandling::errorAdd )
#define DG_ERROR_LE( expr1, expr2 ) \
	DG_CHECK_HELPER_2_( expr1, expr2, LE, DG::ErrorType::RUNTIME_ERROR, ErrAssert, DG::ErrorHandling::errorAdd )
#define DG_ERROR_GE( expr1, expr2 ) \
	DG_CHECK_HELPER_2_( expr1, expr2, GE, DG::ErrorType::RUNTIME_ERROR, ErrAssert, DG::ErrorHandling::errorAdd )

#define DG_ASSERT_TRUE( expr )        \
	DG_CHECK_HELPER_BOOL_(            \
		( expr ),                     \
		#expr,                        \
		false,                        \
		DG::ErrorType::RUNTIME_ERROR, \
		ErrAssert,                    \
		DG::assertHandleCallAdapter )
#define DG_ASSERT_FALSE( expr )       \
	DG_CHECK_HELPER_BOOL_(            \
		!( expr ),                    \
		#expr,                        \
		true,                         \
		...
		ErrAssert,                    \
		DG::assertHandleCallAdapter )
#define DG_ASSERT_EQ( expr1, expr2 ) \
	DG_CHECK_HELPER_2_( expr1, expr2, EQ, DG::ErrorType::RUNTIME_ERROR, ErrAssert, DG::assertHandleCallAdapter )
#define DG_ASSERT_NE( expr1, expr2 ) \
	DG_CHECK_HELPER_2_( expr1, expr2, NE, DG::ErrorType::RUNTIME_ERROR, ErrAssert, DG::assertHandleCallAdapter )
#define DG_ASSERT_LT( expr1, expr2 ) \
	DG_CHECK_HELPER_2_( expr1, expr2, LT, DG::ErrorType::RUNTIME_ERROR, ErrAssert, DG::assertHandleCallAdapter )
#define DG_ASSERT_GT( expr1, expr2 ) \
	DG_CHECK_HELPER_2_( expr1, expr2, GT, DG::ErrorType::RUNTIME_ERROR, ErrAssert, DG::assertHandleCallAdapter )
#define DG_ASSERT_LE( expr1, expr2 ) \
	DG_CHECK_HELPER_2_( expr1, expr2, LE, DG::ErrorType::RUNTIME_ERROR, ErrAssert, DG::assertHandleCallAdapter )
#define DG_ASSERT_GE( expr1, expr2 ) \
	DG_CHECK_HELPER_2_( expr1, expr2, GE, DG::ErrorType::RUNTIME_ERROR, ErrAssert, DG::assertHandleCallAdapter )
...
public:
	...
	DGException(
		...
		DG::ErrorType error_type = DG::ErrorType::RUNTIME_ERROR ) :
		m_err_msg( message ), m_err_code( err_code ), m_error_type( error_type )
	{}
...
private:
	...
	DG::DGErrorID m_err_code;    //!< error code
...
public:
	...
	[[noreturn]] inline static void errorAdd(
		const char *file,
		const char *line,
		const char *func,
		ErrorType type,
	...
	/// \param[in] strm - stream to print critical errors into, but when err_file_path is empty, all errors will be
	/// printed \param[in] err_file_path - file to print all error into; can be empty \return true if there are any
	/// critical error registered
	...
	{
		return get_error_collection().m_err_deque.size();
	}
	...
	inline static void
	assertHandle( const char *file, const char *line, const char *func, const char *expr_str, bool expr );
...
private:
	...
	struct ErrorCollection
	{
		...
		explicit ErrorCollection( size_t max_size = 100 ) :
			m_most_severe_error_type( ErrorType::WARNING ), m_max_size( max_size )
		{}
...
static inline bool errorTypeCheck( const std::exception &e, DG::ErrorType type )
{
	...
	switch( type )
	{
#define _( id, label, desc ) \
case DG::ErrorType::id:      \
	type_str = label;        \
	break;
		DG_ERROR_TYPE_LIST
#undef _
...
public:
	...
	AssertCheckResult(
		...
		const std::array< std::string, N - 1 > &operation,
		const std::array< std::pair< std::string, std::string >, N > &operands ) :
		m_status( status ), m_context( nullptr )
	{
...
#define DG_DEFINE_COMPARISON_HELPER( op_name, op )                                                               \
	...
	inline static AssertCheckResult< 2 >                                                                         \
		CompareHelper##op_name( const T1 &val1, const T2 &val2, const char *expr1, const char *expr2 )           \
	{                                                                                                            \
		if( val1 op val2 )                                                                                       \
...
#define DG_DEFINE_COMPARISON_HELPER_3( op_name1, op1, op_name2, op2 )                                            \
	...
	inline static AssertCheckResult< 3 > CompareHelper##op_name1##op_name2(                                      \
		const T1 &val1,                                                                                          \
		const T2 &val2,                                                                                          \
		const T3 &val3,                                                                                          \
		const char *expr1,                                                                                       \
...
public:
	...
	AssertErrorPrinter(
		...
		const T &result,
		F error_handler ) :
		m_type( type ), m_err_code( err_code ), m_file( file ), m_line( line ), m_func( func ), m_result( result ),
	...
	AssertErrorPrinter &operator=(
		const DG::AssertMessage< std::tuple_size< decltype( T::m_operands ) >::value > &message )
	{
		{
		...
		{
			static_assert(
				std::tuple_size< decltype( m_result.m_operands ) >::value - 1 ==
				std::tuple_size< decltype( m_result.m_operation ) >::value );

			res << "Condition '";
			if constexpr( std::tuple_size< decltype( m_result.m_operands ) >::value == 1 )
			{
				res << m_result.m_operands[ 0 ].first
					<< ( ( m_result.m_operands[ 0 ].second == "true" ) ? " is false" : " is true" );
			}
			else if constexpr( std::tuple_size< decltype( m_result.m_operands ) >::value == 2 )
			{
				res << m_result.m_operands[ 0 ].first << ' ' << m_result.m_operation[ 0 ] << ' '
					<< m_result.m_operands[ 1 ].first;
			}
			else if constexpr( std::tuple_size< decltype( m_result.m_operands ) >::value == 3 )
			{
				res << m_result.m_operands[ 1 ].first << ' ' << m_result.m_operation[ 0 ] << ' '
					<< m_result.m_operands[ 0 ].first << ' ' << m_result.m_operation[ 1 ] << ' '
					<< m_result.m_operands[ 2 ].first;
			}
			else
			{
				static_assert(
...
#define DG_CHECK_HELPER_2_( expr1, expr2, op_str, error_type, error_id, err_handler )                       \
	...
	default:                                                                                                \
		if( const auto ar =                                                                                 \
				DG::AssertCheckResult< 2 >::CompareHelper##op_str( ( expr1 ), ( expr2 ), #expr1, #expr2 ) ) \
			;                                                                                               \
		else                                                                                                \
			DG::AssertErrorPrinter(                                                                         \
				error_type,                                                                                 \
...
#define DG_CHECK_HELPER_3_( expr1, expr2, expr3, op1, op2, error_type, error_id, err_handler )            \
	...
	case 0:                                                                                               \
	default:                                                                                              \
		if( const auto ar = DG::AssertCheckResult<                                                        \
				3 >::CompareHelper##op1##op2( ( expr1 ), ( expr2 ), ( expr3 ), #expr1, #expr2, #expr3 ) ) \
			;                                                                                             \
		else                                                                                              \
			DG::AssertErrorPrinter(                                                                       \
				error_type,                                                                               \
...
inline void DG::ErrorHandling::errorAdd(
	const char *file,
	const char *line,
	const char *func,
	DG::ErrorType type,
...
#undef _
	...
	// throw exception
	throw DGException( full_msg, err_code, type );
}
...
inline void
DG::ErrorHandling::assertHandle( const char *file, const char *line, const char *func, const char *expr_str, bool expr )
{
	if( !expr )
	{
		const std::string msg = std::string( "Assertion failed: '" ) + expr_str + "'. " +
...
inline std::string DG::ErrorHandling::stackTrace( size_t skip, size_t depth )
{
	...
	size_t frames =
		CaptureStackBackTrace( (DWORD)skip, (DWORD)( depth < sizeof stack ? depth : sizeof stack ), stack, NULL );
		...
		if( symInitialized )
		{
				...
				SymGetLineFromAddr64( process, address, &displacement, &line ) )
			{
				frame_desc = DG_FORMAT(
					' ' << std::filesystem::path( line.FileName ).filename().string() << '(' << symbol->Name << ':'
						<< line.LineNumber << ")\n" );
			}
		}
	#endif
