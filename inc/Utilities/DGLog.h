...
#define DG_FAIL_COMMENT( a ) \
	do                       \
	{                        \
		oLog.SetFailed();    \
		oLog << ( a );       \
	} while( 0 )
#define DG_FAIL_COMMENT2( a, b )       \
	do                                 \
	{                                  \
		oLog.SetFailed();              \
		oLog << ( a ) << " " << ( b ); \
	} while( 0 )
#define DG_FAIL_COMMENT3( a, b, c )                    \
	do                                                 \
	{                                                  \
		oLog.SetFailed();                              \
		oLog << ( a ) << " " << ( b ) << " " << ( c ); \
	} while( 0 )
#define DG_FAIL_COMMENT4( a, b, c, d )                                 \
	do                                                                 \
	{                                                                  \
		oLog.SetFailed();                                              \
		oLog << ( a ) << " " << ( b ) << " " << ( c ) << " " << ( d ); \
...
public:
	...
	bool _log( const char *fmt, ... )
	{
		...
		va_list args;
		va_start( args, fmt );
		const size_t msg_len = std::min( (size_t)vsnprintf( msg_buf, sizeof msg_buf, fmt, args ), sizeof msg_buf );
		va_end( args );
...
public:
	...
	explicit DGLog( const char *header, bool reset = false ) : m_header( header ), m_start_time( 0. ), m_timestamp( 0 ), m_status( -1 )
	{
	...
	virtual ~DGLog()
	{
		if( !m_header.empty() || m_status == 0 )
		{
			const std::string log_message = DG_FORMAT(
				"[" << std::left << std::setw( 45 ) << m_header << "]: " << std::fixed << std::setw( 12 ) << ReportInterval()
					<< ( m_status == 0 ? " FAILED " : ( m_status == 1 ? " Passed " : " " ) ) << m_ss.str() << '\n' );
	...
	{
		return get_epoch_time_s() - m_start_time;
	}
