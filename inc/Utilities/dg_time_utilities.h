...
#define get_epoch_time_s DG::TimeHelper::epochTimeGet_s
	...
	/// Get current time as ctime-formatted string (Www Mmm dd hh:mm:ss yyyy\n)
...
bool pollingWaitFor( FUNC f, double timeout_ms )
{
	...
	for( ;; )
	{
		const double time_delta_ms = 1e-3 *
...
public:
	...
	/// Constructor
	/// \param[in] idle_time - idle duration threshold: utilization counters will reset when the period of inactivity
	/// exceeds this duration
	UtilizationTrackerBase( my_duration idle_time = std::chrono::seconds( 1 ) ) :
		m_idle_time_threshold( idle_time ), m_nesting_level( 0 ), m_active_duration( std::chrono::seconds( 0 ) ),
...
private:
	...
	my_duration m_idle_time_threshold;  //!< idle duration threshold: utilization accumulators will reset when the
										//!< period of inactivity exceeds this duration
