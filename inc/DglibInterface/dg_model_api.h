...
/// \return results of execution of requests

json devCtrl( const std::string &server, const json &req );
...
/// \param[in] root_ip is an IP address in the network, formatted as "xxx.xxx.xxx.xxx[:port]". The port part is
/// optional. If included, all servers will be scanned on the port.
///		If omitted, the default port 8778 will be used.
/// \param[in] subnet_mask specifies the subnet with the associated IP, formatted as "xxx.xxx.xxx.xxx", where "xxx" is
/// either 255 for fixed bits or 0 for variable bits. \return Vector of pairs containing IPs of ORCA servers on the
/// network and their statuses
std::vector< std::tuple< std::string, DetectionStatus > > detectSubnetServers(
...
/// \param[in] prefix is a string prefix, formatted as "nnnn[:port]". This will be used to generate two sets of
/// hostnames, one with the pattern "nnnn#", and one "nnnn###". For example, the prefix "farm", range_start 1, range_end
/// 2 and numeral_width 3 will scan the hosts "farm1", "farm001", "farm2", "farm002". Setting numeral_width to 0
/// disables pattern generation, and will just scan the prefix. The port part is optional. If included, all servers will
/// be scanned on the port. If omitted, the default port 8778 will be used. \param[in] range_start is the lowest numeral
/// to be applied to the pattern \param[in] range_end is the highest numeral to be applied to the pattern, inclusive
/// \param[in] numeral_width is the width of the numeral in the padded set. This is optional, set to 3 by default. Set
/// to 0 to disable pattern generation. \return Vector of strings containing hostnames of ORCA servers on the network.
std::vector< std::tuple< std::string, DG::DetectionStatus > > detectHostnameServers(
...
public:
	...
	/// \param[out] json_response is the result of the inference. The response format depends on the model
	/// post-processor type.
...
/// Each of those methods is a non-blocking method, i.e. it returns execution immediately after posting the frame data
/// to the AI server. This allows calling those methods in a loop without waiting for the inference results, achieving
/// the maximum AI server utilization. Once the inference of a frame is complete, and the inference result is received
/// from the AI server, the client callback is invoked to dispatch the inference result. Such result handling via
/// callback mechanism is performed in a thread, separate from the main execution thread. It means that the client
/// callback function is called in asynchronous manner, thus the name of the class.
class AIModelAsync
{
public:
	...
	/// which depends on selected frame format. It can be either JPEG or bitmap depending on the model parameters.
	/// In case of errors throws std::exception.
	/// This is non-blocking call meaning that it returns execution immediately after posting the frame data to the AI
	/// server. \param[in] data is a vector of input data for each model input where each data element is a vector of
	/// bytes. \param[in] frame_info is optional frame information string to be passed to the client callback along with
	/// the frame result. You can pass arbitrary information as frame info. This simplifies matching results to frames
