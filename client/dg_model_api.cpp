...
#include <asio.hpp>
#include <future>
#include "Utilities/dg_string_utilities.h"
...
DG::ModelInfo DG::modelFind( const std::string &server, const ModelQuery &query )
{
	try
	{
		...
		{
			const auto &mparams = m.extended_params;
			if( !DG::Strings::strSearchCI( m.name, query.model_name ) )
				continue;
			if( !query.device_type.empty() && !DG::Strings::strCompareCI( mparams.DeviceType(), query.device_type ) )
				continue;
			if( !query.runtime_agent.empty() &&
				!DG::Strings::strCompareCI( mparams.RuntimeAgent(), query.runtime_agent ) )
				continue;
...
// \param[in] source is an IP address in the network, formatted as "xxx.xxx.xxx.xxx[:port]". The port part is optional.
// If included, all servers will be scanned on the port.
//		If omitted, the default port 8778 will be used.
// \return Vector of strings containing IPs of ORCA servers on the network.
//
static std::vector< std::tuple< std::string, DG::DetectionStatus > > detectServers(
	...
	for( int batch = 0; batch < source.size(); batch += BATCH_SIZE )
	{
		for( int i = batch; i < std::min( batch + BATCH_SIZE, (int)source.size() ); i++ )
			ping_futures[ i ] = std::async(
				std::launch::async,
				[ &result, &result_mutex, connection_timeout_ms ]( const std::set< std::string > &source, int i ) {
					...
					try
					{
						...
						if( ping_result )
							result.push_back(
								std::tuple< std::string, DG::DetectionStatus >( *element, DG::DetectionStatus::OK ) );
						else
							result.push_back( std::tuple< std::string, DG::DetectionStatus >(
								*element,
								DG::DetectionStatus::ProtocolMismatch ) );
					}
					catch( ... )
					{}
				},
...
static std::string hostnameFromPattern( const std::string &pattern, int numeral, const std::string &port )
{
	...
	return port.empty() ? std::string( buffer.get(), buffer.get() + size - 1 )
						: std::string( buffer.get(), buffer.get() + size - 1 ) + ":" + port;
}

//
// Detect all ORCA servers on a given subnet
// \param[in] root_ip is an IP address in the network, formatted as "xxx.xxx.xxx.xxx[:port]". The port part is optional.
// If included, all servers will be scanned on the port.
//		If omitted, the default port 8778 will be used.
// \param[in] subnet_mask specifies the subnet with the associated IP, formatted as "xxx.xxx.xxx.xxx", where "xxx" is
// either 255 for fixed bits or 0 for variable bits. \return Vector of strings containing IPs of ORCA servers on the
// network.
//
std::vector< std::tuple< std::string, DG::DetectionStatus > > DG::detectSubnetServers(
	...
	const std::string &subnet_mask )
{
	...
	for( asio::ip::basic_address_iterator< asio::ip::address_v4 > iterator = subnet.hosts().begin();
		 iterator != subnet.hosts().end();
		 iterator++ )
		source.insert(
			server_port.empty() ? ( *iterator ).to_string() : ( *iterator ).to_string() + ":" + server_port );
...
// 2 and numeral_width 3 will scan the hosts "farm1", "farm001", "farm2", "farm002". Setting numeral_width to 0 disables
// pattern generation, and will just scan the prefix. The port part is optional. If included, all servers will be
// scanned on the port. If omitted, the default port 8778 will be used. \param[in] range_start is the lowest numeral to
// be applied to the pattern \param[in] range_end is the highest numeral to be applied to the pattern, inclusive
// \param[in] numeral_width is the width of the numeral in the padded set. This is optional, set to 3 by default. Set to
// 0 to disable pattern generation. \return Vector of strings containing hostnames of ORCA servers on the network.
//
std::vector< std::tuple< std::string, DG::DetectionStatus > > DG::detectHostnameServers(
	const std::string &prefix,
	const int range_start,
	const int range_end,
	const int numeral_width )
{
